  ...
  return (
    <div className="px-4 py-2">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg sm:text-xl font-semibold">Book Popular Lab Tests</h2>
        <button className="text-blue-600 text-sm hover:text-blue-700">View More</button>
      </div>
      ...
      <div className="flex space-x-2 sm:space-x-4 overflow-x-auto pb-4 scrollbar-hide">
        {config.categories?.map((category, index) => (
          <button
            key={index}
            className={`px-3 sm:px-4 py-1 rounded-full text-sm whitespace-nowrap ${
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 mt-4">
        {config.tests.map((test) => (
          <div key={test.id} className="bg-white rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <button className="px-6 py-1.5 bg-blue-600 text-white rounded-full text-sm hover:bg-blue-700 transition-colors">
                Add
              </button>
            </div>
